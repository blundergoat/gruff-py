...
1. **Justification test.** Any rule, severity, threshold, or score change must reduce at least one of two observable reviewer outcomes on agent-written code: (a) sign-off on a diff carrying a defect the change would have surfaced, or (b) sign-off reached only by reading past the diff — executing the code, reconstructing call graphs, or inferring intent that was never stated. State which of (a) or (b) the change targets. "Matches an industry default" or "is conventionally clean" is not, by itself, a justification.
