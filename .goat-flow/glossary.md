...
- gruff.analysis.v1 = schema string in `AnalysisReport.to_dict()`; the analyse-output contract shared with the PHP/TS/Rust ports.
- gruff.baseline.v1 = schema string for the baseline format (finding-identity ignore list).
- gruff.hotspot.v1 = schema string declared for future hotspot reports; the reporter has not yet shipped.
