...
Applying that rule surfaced a mismatch in the old defaults, where NPATH was
treated as a co-equal error-tier gate beside stronger complexity rubrics:

- **`complexity.npath` was the weakest verification proxy and was Goodhart-fragile.** NPATH was multiplicative by Nejmeh's definition, counting the structural cross-product of branches as if all combinations were reachable. That over-counted exactly the early-return guard-clause / validation-chain idiom — the idiom gruff's own `cyclomatic` and `nesting-depth` remediation strings tell agents to adopt ("replace nested conditionals with early returns", "flatten with guard clauses, early returns"). Concretely, the highest-NPATH function previously observed in gruff-py's own source was `existing_config_source` (`src/gruffpy/command/init_config.py`) at **432** (86% of the 500 gate) — flat guards, nesting ~2, cyclomatic ~9, and unambiguously clean. Raising the threshold would not help: each additional readable guard cost roughly ×3 (`3^6 = 729`, `3^7 = 2187`), so the whole above-gate range up to the 5000 cap was ~2 extra guards. And NPATH was gamed by extracting one guard into a helper — indirection that *lowers* reviewability while the number drops. M01 in the 0.3.0 plan deletes `complexity.npath` outright rather than demoting it to advisory.
...
The surviving headline complexity defaults keep the canonical gruff posture
where it still serves reviewability (cyclomatic 20, cognitive 30, nesting 6 —
all severity error). The NPATH deletion is not a "the number is wrong" change;
it is a "the metric is the wrong forcing signal for the agent-hook use case"
change.
