...
- **Analysis JSON gains a `suppressions` array** - One `{index, rule, paths, symbol, reason, suppressed}` audit row per configured exclusion, including entries that matched nothing, so a suppressed finding is counted rather than invisible. The array is `[]` when nothing is configured, and `analyse --format text` prints the same total under `Sensitive exclusions`. `gruff.analysis.v2` is unchanged for consumers reading `findings`.
- **`summary` text now states its suppressed total** - `summary` already applied `sensitiveExclusions`, but printed no audit row; its text output now carries the same `Sensitive exclusions` section `analyse` prints, below the canonical `Composite:`/`Findings:` block. `summary --format json` still filters without publishing a count until the `gruff.summary.v2` envelope gains a suppression surface; that envelope is unchanged here.
