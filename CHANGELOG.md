...
- **BREAKING: every `sensitive-data.*` rule defaults to `warning`** - Six rules defaulted to `error`: `aws-access-key`, `database-url-password`, `gcp-service-account-key`, `phi-pattern`, `private-key` and `url-credentials`. The family unified sensitive-data severity at warning. A detector that hard-fails a build on a match it cannot confirm teaches users to add blanket ignores, which is how a secret scanner ends up switched off. A `--fail-on error` gate no longer fails on a secret finding. `--fail-on warning` and the advisory floor still do, and a rule's `severity: error` in `.gruff-py.yaml` restores the old behaviour. Scores move with the severity weight.
...
- **Fixed: a configuration error publishes its envelope when the target is outside the launch directory** - `analyse ../proj --format json` from a sibling directory, with a `--config` that could not load, exited `1` with a `ValueError` traceback and printed nothing on stdout, because the failure envelope is rooted at the launch directory and the target has no path under it. The envelope now leaves that input out, as it already left out the config path, and the run exits `2` with its `config-error` diagnostic.
- **Fixed: a target named from a project subdirectory is read from there** - `analyse ..`, `summary ..` and `report ..` run from one of the project's own subdirectories read the operand against the project root instead of the launch directory. They scanned the directory above the project and raised on its first file with no JSON on stdout. Each relative target is now read from the launch directory, so these runs give the same result as one from inside the project.
- **Fixed: a symbol containing `#` no longer aborts a baseline** - `#` joins a symbol to its declaration ordinal, so a finding whose symbol already held one made `--generate-baseline` and `--baseline` fail. The finding is now reported with `baselineStatus: notEligible`, no SARIF `partialFingerprints` and a null hook `stableIdentity`; it is never stored and never counted as a secret.
- **BREAKING: `list-rules --format json` publishes every threshold as a named knob map** - A single-threshold rule's value moves from the scalar `threshold` to `thresholds`, as `{"maxLines": 100}` where gruff-go already names the knob and `{"threshold": 10}` where no port does; named-knob rules keep their map, and a rule with no threshold publishes neither key. `list-rules <rule_id> --format json` changes the same way. SARIF rule properties and `.gruff-py.yaml` keys are unchanged; this is the shape all five ports publish at 0.6.0.
- **BREAKING: the per-command exit gate moves from `minimumSeverity:` to `failOn:`** - A config carrying the per-command `minimumSeverity:` map is refused with exit `2`, naming `failOn` as the key that gates the exit code. `failOn` accepts `analyse`, `report` and `dashboard` in both `.gruff-py.yaml` and `[tool.gruff-py.failOn]`; `minimumSeverity` survives as a scalar display floor that sets the `--min-severity` default without changing the exit code or the score, and it takes `advisory`, `warning` or `error` rather than `none`. The split is the ratified family contract (`gruff-spec/contracts/core/cli.v1.json`, ratified 2026-09-06), chosen so a configuration can never silently gate. `gruff-py migrate-config` renames the key and preserves the original file.
- **BREAKING: the agent-hook contract moves from `gruff.hook.v1` to `gruff.hook.v2`** - The payload's `contractVersion` changes, and the envelope gains two required keys: `run`, carrying the audit data a consumer needs to trust the verdict (mode, scope, operands, `analysedFiles`, and the applied baseline), and `suppressions`, one row per configured sensitive exclusion with the count it suppressed in this run, `0` when the entry matched nothing. The exits are ratified as three and no others: `0` when nothing reached the gate, `1` when something did under an explicit consumer request (`--fail-on`, `--fail-on-new`, or `--fail-on-diagnostics`), and `2` when the run could not happen. A consumer reading the v1 seven-key payload keeps working; one that validates the key set must accept the two new keys. The contract is `gruff-spec/contracts/core/hook.v2.json`, ratified 2026-09-06.
- **BREAKING: baselines move to the family `gruff.baseline.v3` file, and every finding identity changes once** - A baseline row now stores one line-free identity and a count: sha256 over the tool language, native rule id, project-relative path, and a subject that is the symbol plus its declaration ordinal, or, when no symbol is named, the message with its measured values normalised. Inserting code above a reviewed finding no longer expires its entry, two same-named declarations in one file can no longer share a review, and a second occurrence beyond the reviewed count is reported as new rather than hidden. A `gruff-py.baseline.v1` or `gruff.baseline.v1` file fails closed and names the migration command.
- **`analyse --migrate-baseline <old> --generate-baseline-path <new>` carries a 0.5 baseline forward** - The reviewed findings are re-identified from the current scan and written to a separate file; the original is never written to, renamed, or deleted, and an output that is the input by spelling, symlink, or hard link is refused. A 0.5 file naming more than one row container is refused too, because the five 0.5 writers used three container keys and such a file would migrate differently in different ports. A refused migration writes nothing and leaves the original byte for byte. Nothing converts a file in place.
...
- **BREAKING: `sensitive-data.high-entropy-string` adopts the family contract** - It reports at medium confidence rather than low, and `thresholds.minLength` (default `32`) and `thresholds.entropy` (default `4.2`) are now configurable where `20` and `4.5` were hardcoded. Tokens of 20 to 31 characters stop reporting, tokens between 4.2 and 4.5 bits per character start, and each finding weighs more in the score. To keep the old bar, set `minLength: 20` and `entropy: 4.5` under the rule's `thresholds`.
- **`sensitive-data.hardcoded-env-value` reads a value from its own line only** - `AWS_SECRET_ACCESS_KEY=` with nothing after it no longer takes the next line as its value. Placeholders match in any casing and include `your-...`, `<...>` and a single repeated character, and a value in `.env.example`, `.env.sample`, `.env.template` or `.env.dist` reports only when it looks like a generated credential.
- **`sensitive-data.url-credentials` ignores template userinfo** - A password segment holding `{`, `}`, `\`, `/`, `:` or a `%` that is not an escape no longer reports, and a URL repeated on the next line reports once; short dummy passwords such as `pass` still report.
- **`test-quality.no-assertions` reports only tests a runner collects** - Files must match pytest's `python_files` from `pyproject.toml` or unittest's `test*.py`; shipped package modules, `nox` sessions and `invoke` tasks are skipped, and assertions inside nested functions count.
- **`dead-code.exported-but-unreferenced` counts references from ignored paths** - A symbol used only under `paths.ignore`, such as an ignored `tests/` tree, is no longer reported as unreferenced; ignored files still never produce a finding.
- **`naming.boolean-prefix` accepts an auxiliary marker anywhere in a name** - `state_is_attributed` and `sentence_needs_review` pass, and the finding message now lists the vocabulary the rule actually accepts.
- **`waste.commented-out-code` judges comment blocks** - Lines under a task-marker header, quoted examples under a header such as `For example:`, and a function name that ends a sentence no longer report; a tombstoned function still reports per line.
- **`test-quality.mystery-guest` reports only calls that perform I/O** - Constructors such as `requests.Session()`, `open` in a test that takes `tmp_path`, and calls on a parameter named like a module no longer report.
- **`docs.ignore-directive-reason` skips generated files and accepts issue references** - A header saying the file is generated skips it, `mypy#4125` or `GH-1234` counts as a reason, and `# noqa:F401, isort:skip` is reported without the neighbouring directive read as a code.
- **`design.runtime-sys-path-mutation` lets a standalone script change `sys.path` at its top level** - A file with a `__main__` guard, a shebang or a home under `scripts/`, `bin/` or `tools/` is exempt there; a mutation inside a function still reports.
- **A baseline applies in any checkout** - `docs.missing-readme` stored an absolute path in its baseline row, so a committed baseline reported it again on every other machine; the row is now `README.md`.
- **Baseline errors and tips print the canonical flags** - Refusals, tips and diagnostics name `--baseline` and `--generate-baseline`, so a copied command no longer raises the superseded-spelling warning.
- **The `minimumSeverity:` display floor is recorded in JSON** - Like `--min-severity`, the configured floor now publishes `displayFilter` with the count it hid; the score and exit code are unchanged.
- **Known limitation: the `cryptography` speed-up measured on 2026-08-12 has no family regression suite** - The scan that fell from 522 s to 15 s ships unguarded by the family, because the suite that would hold it was deferred past 0.6.0.
- **The self-scan gate proves zero findings, not zero false positives** - `gruff-py` finding nothing in its own repository is an invariant this release holds, not evidence about false-positive rates, which negative fixtures measure and a self-scan cannot.
