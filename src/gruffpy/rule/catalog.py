...
_REVIEWED_FALSE_POSITIVE_GUIDANCE: dict[str, tuple[str, str]] = {
    "complexity.halstead-volume": (
        "A declarative builder dominated by one literal table can have high token volume "
        "despite having little control-flow complexity.",
        "Extract the table to data or tune this rule's `threshold` and `severity` when "
        "the volume is an accepted project convention.",
    ),
    "complexity.maintainability-index": (
        "A long but linear wiring, data-setup, or generated function can score poorly even "
        "when its branching is simple.",
    ...
    "dead-code.unused-private-attribute": (
        "A private attribute read through `getattr`, serialization, or framework reflection "
        "has no parser-visible load.",
        "Keep an explicit read when practical, or suppress "
        "`dead-code.unused-private-attribute` with a reason at the reviewed declaration.",
    ),
    ...
    "docs.complex-branch-rationale": (
        ...
        "Add a substantive local rationale, or tune the public/private cyclomatic and "
        "cognitive warning options to the project's documented policy.",
    ),
    "docs.dataclass-attributes": (
        "Dataclass fields documented by an inherited schema or external generator are not "
        "visible in the class's local docstring.",
    ...
    "docs.missing-module-docstring": (
        "A generated or deliberately single-purpose production module can be documented by "
        "its package reference instead of a module docstring.",
        "Add the local module summary, exclude generated paths from analysis, or suppress the "
        "rule for the reviewed module with a reason.",
    ),
    ...
    "docs.missing-raises-doc": (
        "A wrapper can re-raise an exception whose contract is documented on the interface it "
        "implements rather than in its local docstring.",
        "Add a local Raises section, or suppress the rule on the reviewed wrapper when the "
        "inherited exception contract is authoritative.",
    ),
    "docs.missing-readme": (
        "A package can use `docs/index.md` or another generated landing page as its maintained "
        "entry point instead of a root README.",
    ...
    "docs.missing-return-doc": (
        "An override can inherit its return contract from a documented Protocol or base class "
        "while omitting a local Returns section.",
        "Repeat the return contract locally, or suppress the rule on the reviewed override "
        "when inherited documentation is authoritative.",
    ),
    "docs.todo-density": (
        "A planning or migration module can intentionally carry several tracked TODO markers "
        "while the work remains bounded and owned.",
        "Move the work to the issue tracker or tune this rule's `threshold` and `severity` for "
        "the reviewed planning surface.",
    ),
    "docs.useless-docstring": (
        "A protocol adapter or command method can have a deliberately terse summary whose "
        "meaning is supplied by a stable interface.",
        "Add the missing behavior or constraint to the summary, or tune "
        "`options.min_summary_words` for the project's documentation convention.",
    ),
    ...
    "naming.boolean-prefix": (
        ...
        "Add the exact contract name to `options.acceptedBooleanNames` instead of renaming the "
        "external surface.",
    ),
    "naming.confusing-name": (
        "A deliberately short class name can gain its missing domain context from the enclosing "
        "module or package.",
    ...
    "naming.generic-function": (
        ...
        "Remove the required hook name from `options.genericFunctions` or suppress the finding "
        "at the reviewed implementation.",
    ),
    "naming.module-name-mismatch": (
        "A feature-oriented module can intentionally contain one public class plus supporting "
        "functions without being named after that class.",
        "Add the module name to `options.conventionalModuleNames`, reorganize the module, or "
        "suppress the reviewed exception.",
    ),
    "naming.short-variable": (
        "A one-character domain or mathematical symbol can be conventional even when it is not "
        "one of the built-in loop, axis, or exception names.",
        "Add the exact symbol to `options.acceptedShortNames` with the project's convention, or "
        "rename it where the short form is not load-bearing.",
    ),
    "naming.test-naming-consistency": (
        "A compatibility or migration test file can intentionally preserve both unittest-style "
        "camelCase names and pytest-style snake_case names.",
        "Finish the rename when compatibility permits, or suppress the file-level advisory with "
    ...
    "security.django-mark-safe": (
        ...
        "Pass the value through an explicit escaping helper or `format_html`, or suppress the "
        "reviewed sink with the upstream safety reason.",
    ),
    "security.error-suppression": (
        "A best-effort cleanup or telemetry path can deliberately suppress every exception "
        "because failure must not replace the primary result.",
    ...
    "security.extract-compact-user-input": (
        "A request mapping splat can contain only allowlisted keys after validation performed "
        "outside the expression this rule sees.",
        "Copy validated keys into an explicit dictionary before `**` expansion, or suppress the "
        "reviewed call with the validation contract.",
    ),
    "security.github-actions-secrets-in-pr": (
        "A secret reference in a pull-request workflow can sit behind a trusted-actor condition "
        "that the workflow text scan does not evaluate.",
    ...
    "security.hardcoded-bind-all-interfaces": (
        "A containerized service can deliberately bind to all interfaces while an external "
        "network policy prevents public exposure.",
        "Read the bind address from deployment configuration, or suppress the reviewed literal "
        "with the network-boundary reason.",
    ),
    "security.header-injection": (
        ...
        "Map the validated choice to literal header assignments, or suppress the reviewed sink "
        "with the allowlist evidence.",
    ),
    "security.insecure-random": (
        ...
        "Use `secrets` when unpredictability matters; otherwise rename the non-security value or "
        "suppress the reviewed call with its purpose.",
    ),
    "security.insecure-temp-file": (
        "A fixed temporary path can be process-private inside an isolated sandbox even though "
        "that exclusivity is not visible at the call site.",
        "Use `mkstemp` or `NamedTemporaryFile`, or suppress the reviewed path with evidence of "
        "the containing permission and lifecycle controls.",
    ),
    ...
    "security.variable-import": (
        "A dynamic module name can come from a closed plugin registry or internal allowlist that "
        "the import expression does not expose.",
        "Map allowed names to explicit imports, or suppress the reviewed import with the registry "
        "boundary that prevents user control.",
    ),
    "sensitive-data.hardcoded-env-value": (
        "A non-secret value can use a key containing PASS, TOKEN, or SECRET while its length and "
        "entropy resemble a credential.",
    ...
    "sensitive-data.high-entropy-string": (
        ...
        "Replace fixtures with a recognizable placeholder, or add a reasoned "
        "`sensitiveExclusions` entry for the exact reviewed rule and path.",
    ),
    "sensitive-data.phi-pattern": (
        "A structurally valid synthetic SSN or labelled MRN outside the placeholder set can look "
        "like real health data.",
        "Use a recognized placeholder, or add a reasoned `sensitiveExclusions` entry for the "
        "exact reviewed rule and fixture path.",
    ),
    "sensitive-data.pii-test-fixture": (
        ...
        "Use `example` domains or 555-style numbers, or add a reasoned `sensitiveExclusions` "
        "entry for the exact reviewed rule and fixture path.",
    ),
    "test-quality.conditional-logic": (
        "A property, state-machine, or compatibility test can intentionally exercise several "
        "outcome branches in one named scenario.",
        "Parametrize or split the branches for separate failures, or suppress the reviewed test "
        "when one scenario must retain the control flow.",
    ),
    "test-quality.eager-test": (
        "One outcome can require many field-level assertions, so assertion count alone can make "
        "a focused contract test look eager.",
        "Compare a structured expected value or tune `thresholds.maxAssertions` after confirming "
        "all assertions describe the same behavior.",
    ),
    "test-quality.exception-type-only": (
        "A boundary can intentionally promise only that any wide exception escapes, with no "
        "stable message suitable for `match=`.",
        "Prefer a narrow exception or stable message match; otherwise suppress the reviewed "
        "assertion with the type-only contract.",
    ),
    "test-quality.excessive-mocking": (
        "A coordinator test can legitimately isolate several collaborators because orchestration "
        "is the production responsibility under test.",
        "Use a higher-level test or tune `thresholds.maxMocks` for suites whose reviewed subject "
        "is an orchestrator.",
    ),
    "test-quality.global-state-mutation": (
        "A test can restore a `global` binding in a fixture or `finally` block, but this rule "
        "reports the declaration without evaluating cleanup.",
        "Use `monkeypatch` or a fixture-owned state boundary, or suppress the reviewed test with "
        "the cleanup guarantee.",
    ),
    "test-quality.loop-assertion-without-message": (
        "The compared item can already have a stable, descriptive repr that identifies the "
        "failing iteration without a custom assertion message.",
    ...
    "test-quality.loop-in-test": (
        "A state-machine, fuzz, or property test can require a loop whose branches make the "
        "built-in fixture-loop exemption inapplicable.",
        "Parametrize finite cases, or suppress the reviewed test when iteration is itself the "
        "behavior under test.",
    ),
    ...
    "test-quality.mock-only-test": (
        "An interaction can be the complete contract, such as proving an event was dispatched or "
        "a boundary received a mapped payload.",
    ...
    "test-quality.mock-without-expectation": (
        ...
        "Use a small fake or add an explicit `assert_not_called` expectation so the intent is "
        "visible to the rule and reviewer.",
    ),
    "test-quality.mocking-domain-object": (
        "A configured domain namespace can also contain ports or interfaces that are appropriate "
        "mock boundaries.",
        "Narrow `options.domain_namespaces` to concrete domain-object packages or use a small fake "
        "for the reviewed port.",
    ),
    "test-quality.multiple-aaa-cycles": (
        "One workflow or state-transition test can deliberately assert after each step, making "
        "several call-separated assertion blocks one scenario.",
        "Split the transitions or tune `thresholds.maxCycles` after confirming the test is one "
        "reviewed workflow.",
    ),
    "test-quality.mystery-guest": (
        "A test can call `open` on `tmp_path` or reach a local test server even though the target "
        "is fixture-owned and hermetic.",
        "Move the I/O behind an explicit fixture/helper or suppress the reviewed test with the "
        "fixture boundary.",
    ),
    "test-quality.naming-consistency": (
        "A compatibility or staged migration suite can intentionally mix pytest function names "
        "with unittest-style function or class names.",
        "Complete the rename when compatibility permits, or suppress the file-level advisory with "
        "the migration reason.",
    ),
    "test-quality.parametrize-annotation": (
        ...
        "Add explicit IDs or tune `thresholds.maxCasesWithoutIds` after checking the actual report "
        "names.",
    ),
    "test-quality.private-reflection": (
        "A compatibility, serialization, or migration test can deliberately verify a private "
        "attribute as the persisted contract.",
        "Prefer public behavior, or suppress the reviewed access with the compatibility "
        "contract it protects.",
    ),
    "test-quality.pytest-coverage-source-missing": (
        "Coverage source can be supplied by CI arguments, `.coveragerc`, or environment settings "
        "that the pyproject-only check does not read.",
        "Declare `[tool.coverage.run]` source, or suppress the project finding after checking "
        "the external coverage command.",
    ),
    "test-quality.pytest-deprecations-not-fatal": (
        ...
        "Declare the error filter in `[tool.pytest.ini_options].filterwarnings`, or suppress the "
        "project finding after verifying the external gate.",
    ),
    "test-quality.pytest-strict-config-missing": (
        ...
        "Add both flags to `[tool.pytest.ini_options].addopts`, or suppress the finding with "
        "the verified external gate or compatibility reason.",
    ),
    "test-quality.repeated-structure-missing-parametrize": (
        ...
        "Parametrize the cases or tune `thresholds.minGroupSize`; suppress the group when separate "
        "scenario identities are intentionally retained.",
    ),
    "test-quality.setup-bloat": (
        "A costly domain fixture can be longer than its deliberately small focused tests without "
        "representing accidental shared state.",
        "Extract factories or tune `thresholds.maxSetupLines` after confirming the shared setup is "
        "the reviewed suite boundary.",
    ),
    "test-quality.sut-not-called": (
        "A contract test can validate module constants or metadata using only builtins and test "
        "helpers, with no callable system under test.",
        "Assert through the public contract where possible, or suppress the reviewed test with the "
        "declarative surface it verifies.",
    ),
    "test-quality.tautological-type-assertion": (
        ...
        "Evaluate the call once and assert the specific expected type, or suppress the reviewed "
        "dynamic-factory case.",
    ),
    "test-quality.test-longer-than-sut": (
        "A table-driven contract test can need many scenarios around one short same-file "
        "function, so line ratio overstates duplication.",
        "Split or extract setup, or tune `options.ratio` after reviewing the scenario coverage.",
    ),
    "test-quality.trivial-snapshot": (
        "A canonical encoding or compatibility fixture can intentionally pin every element of a "
        "large literal collection.",
        "Assert salient invariants, or suppress the reviewed snapshot with the exact "
        "compatibility contract it protects.",
    ),
    "waste.commented-out-code": (
        "Explanatory prose can itself be valid Python after the code-like prefilter, such as a "
        "comment written as an assignment or call.",
        "Rewrite it as prose that does not parse as a statement, or suppress the reviewed comment "
        "with its documentation purpose.",
    ),
    "waste.one-line-function": (
        "A strict passthrough wrapper can still be a stable typing, dispatch, monkey-patching, or "
        "public compatibility boundary.",
        "Keep the reviewed wrapper when that boundary is intentional; otherwise inline the call.",
    ),
    "waste.unused-parameter": (
        "An unrecognized callback or protocol can require a parameter that is consumed indirectly "
        "through `locals()` or reflection.",
        "Prefix the name with `_` to declare it intentionally unused, or suppress the reviewed "
        "signature when the external protocol fixes the name.",
    ),
}
