            ...
            for fixture_path in sorted(expectation_dir.rglob("*.py")):
                ...
                cases.append(
                    pytest.param(
                        ...
                        expectation,
                        fixture_path,
                        id=f"{rule_dir.name}:{expectation}:{fixture_name}",
                    )
                )
    return cases
...
@pytest.mark.parametrize(
    ("rule_id", "expectation", "fixture_path"),
    _fixture_cases(),
)
def test_calibration_fixture(rule_id: str, expectation: str, fixture_path: Path) -> None:
    ...
    matching_findings = [finding for finding in findings if finding.rule_id == rule_id]
