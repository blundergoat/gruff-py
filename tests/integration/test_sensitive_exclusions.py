...
_SYNTHETIC_JWT = "eyJ" + "hbGciOiJIUzI1NiJ9" + "." + "eyJzdWIiOiJzeW50aGV0aWMifQ" + "." + "c3ludGhldGljc2ln"
...
def _write_corpus(project_root: Path) -> None:
    ...
    (secrets / "aws.env").write_text(f"AWS_KEY={_SYNTHETIC_AWS_KEY}\nAWS_BACKUP_KEY={_SYNTHETIC_AWS_KEY}\nSESSION_TOKEN={_SYNTHETIC_JWT}\n")
...
def _finding_scopes(payload: dict[str, Any]) -> set[tuple[str, str, int | None]]:
    ...
    return {(finding["ruleId"], finding["file"], finding["line"]) for finding in payload["findings"]}
...
def test_two_distinct_entries_each_report_their_own_count(corpus: Path) -> None:
    ...
    removed = _finding_scopes(baseline) - _finding_scopes(configured)
    assert removed == {
        scope for scope in _finding_scopes(baseline) if (scope[0], scope[1]) in {(_AWS_RULE, "secrets/aws.env"), (_JWT_RULE, "secrets/jwt.env")}
    }
...
def test_same_rule_other_file_survives_when_only_the_sibling_is_excluded(corpus: Path) -> None:
    ...
    assert (_AWS_RULE, "secrets/aws.env") in {(finding["ruleId"], finding["file"]) for finding in configured["findings"]}
...
def test_suppressed_findings_leave_the_score_and_exit_code(corpus: Path) -> None:
    ...
    assert not [finding for finding in payload["findings"] if finding["ruleId"] in {_AWS_RULE, _JWT_RULE}]
...
def test_text_output_states_the_suppressed_total_and_its_rationale(corpus: Path) -> None:
    ...
    assert (f"  Suppressed findings: 2 via sensitiveExclusions[0] {_AWS_RULE}: 2 (Synthetic AWS key used by the redaction corpus.)") in result.stdout
...
def test_summary_text_states_the_same_suppressed_total_as_analyse(corpus: Path) -> None:
    ...
    audit_line = f"  Suppressed findings: 2 via sensitiveExclusions[0] {_AWS_RULE}: 2 (Synthetic AWS key used by the redaction corpus.)"
...
def test_no_reported_field_carries_matched_value_material(corpus: Path) -> None:
    ...
    result = CliRunner().invoke(main, ["analyse", "--format", "json", "--fail-on", "none", "secrets"])
